                        ...
                        @foreach ($users as $user)
                            <tr class="table-{{ $user->status['badge'] }}">
                                ...
                                <td class="">
                                    ...
                                    <a href="https://api.whatsapp.com/send?phone={{ $user->userProfile->phone_number }}&text=Assalamualaikum Wr Wb. %0A %0A Yth Direktur *TPA {{ $user->userProfile->institution_name }}*, %0A %0AMengharap keikutsertaan TPA {{ $user->userProfile->institution_name }} dalam acara *Badko Kasihan Berkisah,* bersama *Kak Bimo* (Juru Kisah Nasional) %0A%0AInsyaAllah diadakan pada :%0A🗓 Ahad, 16 Februari 2025%0A⏰ 07.00-selesai%0A🏦 Gedung Griyo Nirmolo (Kalurahan Tirtonirmolo), Kasihan%0A%0A📞 Undangan dan informasi selengkapnya di https://rsvp.walima.id/kasihan-berkisah?n={{ $user->userProfile->institution_name }} %0A%0ADemikian undangan ini kami sampaikan,. atas perhariannya kami ucapkan jazakumullahu khairan.%0A%0AHormat Kami, %F0%9F%98%8A %0A Badko TKA-TPA Kasihan"
